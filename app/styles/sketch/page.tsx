...
function Navbar() {
  return (
    <nav
      ...
      style={{ borderColor: tokens.border, backgroundColor: tokens.background, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="px-5 h-10 text-sm font-medium border-2 border-transparent"
          style={{ backgroundColor: tokens.accent, color: tokens.accentForeground, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}
        >
          Get started
        </motion.button>
      </div>
        ...
        >
          ...
          >
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="h-14 px-8 font-medium flex items-center gap-2 border-2 border-transparent"
              style={{ backgroundColor: tokens.accent, color: tokens.accentForeground, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}
            >
            ...
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="h-14 px-8 font-medium border-2"
              style={{ borderColor: tokens.border, color: tokens.foreground, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}
            >
              View demo
            </motion.button>
          </motion.div>
...
function Stats() {
  return (
    <section className="border-y" style={{ borderColor: tokens.border, backgroundColor: tokens.backgroundAlt, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {FEATURES.map(feature => (
              <motion.div
                ...
                className="p-8 border-2 hover:rotate-1 rotate-[-1deg] transition-transform duration-100"
                style={{ borderColor: tokens.border, backgroundColor: tokens.backgroundAlt, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}
              >
...
function ProductDetail() {
  return (
    <section className="py-24 border-y" style={{ borderColor: tokens.border, backgroundColor: tokens.backgroundAlt, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {PRICING.map(tier => (
              <motion.div
                key={tier.name}
                variants={staggerItem}
                className="p-8 border-2 relative hover:-rotate-1 rotate-[1deg] transition-transform duration-100"
                style={{ borderColor: tier.highlighted ? tokens.accent : tokens.border, backgroundColor: tier.highlighted ? `${tokens.accent}08` : tokens.backgroundAlt, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: tier.highlighted ? `4px 4px 0px 0px ${tokens.accent}` : '4px 4px 0px 0px #2d2d2d' }}
              >
                {tier.highlighted && (
                  <span
                ...
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="w-full h-12 font-medium text-sm border-2"
                  style={tier.highlighted ? { backgroundColor: tokens.accent, color: tokens.accentForeground, borderColor: tokens.accent, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' } : { backgroundColor: 'transparent', color: tokens.foreground, borderColor: tokens.border, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}
                >
                  {tier.cta}
                </motion.button>
              </motion.div>
...
function Testimonials() {
  return (
    <section id="testimonials" className="py-24 border-y" style={{ borderColor: tokens.border, backgroundColor: tokens.backgroundAlt, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}>
      <div className="max-w-6xl mx-auto px-6">
        <FadeUp>
          <div className="text-center mb-16">
        ...
        <div className="space-y-4">
          {FAQ_ITEMS.map((item, i) => (
            <FadeUp key={i} delay={i * 0.05}>
              <div className="border-2 overflow-hidden hover:rotate-1 transition-transform duration-100" style={{ borderColor: tokens.border, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}>
                <button
          ...
          {status === 'success' ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
              ...
              <input
                ...
                className="flex-1 h-12 px-4 border-2 text-sm"
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="h-12 px-6 font-medium text-sm disabled:opacity-60 border-2 border-transparent"
                style={{ backgroundColor: tokens.accent, color: tokens.accentForeground, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}
              >
  ...
  return (
    <footer className="py-16" style={{ backgroundColor: tokens.background, borderTop: `1px solid ${tokens.border}` }}>
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t gap-4" style={{ borderColor: tokens.border, borderRadius: '255px 15px 225px 15px / 15px 225px 15px 255px', boxShadow: '4px 4px 0px 0px #2d2d2d' }}>
